...
void insertionSort(int arr[], int n) {
    ...
    for(int i=0; i<n; i++) {
        ...
        int prevIdx = i-1;    // Start comparing from the previous element

        // Move elements of the sorted segment forward if they are greater than curr
        while (prevIdx >=0 && arr[prevIdx] > curr) {
            swap(arr[prevIdx], arr[prevIdx+1]); // Swap the previous element with the current to shift larger elements forward
            prevIdx--; // Move to the next previous element for comparison
        }
    }
}
