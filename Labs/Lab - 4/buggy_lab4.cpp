...
void identifyMeanAndMin(const std::string& filename) {
   ...
   *sum = 0;
   ...
   for (int i = *numElements-1; i >= 0; i--) {
    // for(int j = numElements[i] -1; j >=0; j--) {
       ...
       if (i == *numElements-1) {
           *smallestNum = *(intArray + i); // *(intArray + i) is eqaul to saying intArray[i]
    ...
    // writing the for loop in non-pointer syntax
