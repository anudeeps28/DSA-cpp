...
class Array
{
private:// building blocks of the array
    int *A;
    int size;
    int length;
...
public: // definitions
    Array() // general constructor
    {
        size=10;
        length=0;
        A=new int[size];
    }
    Array(int sz) // parametrized constructor for making array of any size
    {
        size=sz;
        length=0;
        A=new int[size];
    ...
    // all the functions are a part of the class
    void Display();
    void Append(int x);
...
/*we write as "void class_name::function_name() 
{
    *function
}
*/
...
// function to insert an element to an index position
...
// function to delete and element at an index
...
// get function to get an element from an index
...
// set function to set the value of an element of an index
...
// finding minimum from a function
int Array::Min()
{
    int min=A[0];
    int i;
